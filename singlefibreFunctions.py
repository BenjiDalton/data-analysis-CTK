...
def rFEAnalysis(data: pd.DataFrame=None, metaData: dict=None, graph: plt.Axes=None, graphColor: Colors=None, label: str=None) -> tuple[pd.DataFrame, float, float, float]:
    ...
    graphData=pd.DataFrame(data[['Time (ms)', 'Force in (mN)']])
    graphData['Time (ms)']=graphData['Time (ms)']
    ...
    graph.plot(graphData['Time (ms)'], graphData['Force in (mN)'], color=graphColor, linewidth=graphLinewidth, label=label)
...
def getContractionData(data: pd.DataFrame=None, idx: int=None, protocolInfo: dict=None) -> pd.DataFrame:
    ...
    data=data.loc[lengthWindow, ['Time (ms)', 'Length in (mm)', 'Force in (mN)']]
...
def Binta_Analysis(data: pd.DataFrame=None, metaData: dict=None, graph: plt.Axes=None) -> dict:
    ...
    if metaData['filename info']['protocol'].upper() == 'RFD':
        forceDepressionResults, isoResults=map(lambda data, color, label: rFEAnalysis(data, metaData, graph, color, label), [firstFrameData, secondFrameData], [Colors.Black, Colors.Firebrick], ['rFD', 'ISO'])
        analysisResults=forceDepressionResults,isoResults
    if metaData['filename info']['protocol'].upper() == 'RFE':
        isoResults, forceEnhancementResults=map(lambda data, color, label: rFEAnalysis(data, metaData, graph, color, label), [firstFrameData, secondFrameData], [Colors.Black, Colors.Firebrick], ['ISO', 'rFE'])
        analysisResults=isoResults, forceEnhancementResults
...
def Makenna_Analysis(data: pd.DataFrame=None, metaData: dict=None, graph: plt.Axes=None) -> dict:
    ...
    if metaData['filename info']['protocol'].upper() == 'SSC':
        ...
        stretchWork, shortenWork=map(
            lambda data, lineColor, label, annotationOffset: workCalculation(data, graph=graph, graphLinecolor=lineColor, graphLabel=label, annotationOffset=annotationOffset), 
                [stretchData, shortenData], 
                [Colors.Firebrick, Colors.DeepBlue], 
                ['Stretch', 'Shorten'], 
                [-100, 20])
        netWork=shortenWork + stretchWork
        analysisResults=[peakEccentricForce, stretchWork, shortenWork, netWork, stiffness]

    if metaData['filename info']['protocol'].upper() == 'ISO':
        peakForce=np.mean(data['Force in (mN)'].loc[stiffnessTime - 5001:stiffnessTime-1])

        analysisResults=[peakForce, stiffness]

    if metaData['filename info']['protocol'].upper() == 'CONCENTRIC':
        shortenData=getContractionData(data=data, idx=0, protocolInfo=protocolInfo)
        shortenWork=workCalculation(data=shortenData, graph=graph, graphLinecolor=Colors.Firebrick, graphLabel='Shortening')
        forceFollowingShortening=np.mean(data['Force in (mN)'].loc[stiffnessTime - 5001:stiffnessTime-1])

        analysisResults=[shortenWork, forceFollowingShortening, stiffness] 
